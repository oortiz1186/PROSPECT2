...
import { DEFAULT_WHATSAPP_TEMPLATE, renderTemplate, buildWhatsAppUrl } from "@/lib/message";

export async function POST(request: Request) {
  const { prospectId, template } = await request.json();
  ...
  const body = renderTemplate(template || DEFAULT_WHATSAPP_TEMPLATE, {
